...
- **Deploy GitHub Pages** (unico passo rimasto della Fase 1): non esiste ancora un remote GitHub. Serve: creare il repo, `vite.config.ts` con `base: '/<nome-repo>/'`, workflow o `gh-pages`, push. **Richiede conferma utente** (pubblicazione).
...
- [ ] **Deploy GitHub Pages** — unico passo rimasto della Fase 1 (serve conferma utente + creazione repo).
