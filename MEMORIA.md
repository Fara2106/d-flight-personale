...
- **Fase 1 CHIUSA, deploy FATTO (2026-07-03):** app online su **https://fara2106.github.io/d-flight-personale/** (repo pubblico `Fara2106/d-flight-personale`, confermato dall'utente). Deploy automatico a ogni push su `main` via `.github/workflows/deploy.yml` (typecheck + test + build + Pages). Nota: la **prima** deployment di un repo appena creato fallisce con "Deployment failed, try again later" (race di provisioning Pages) — basta rilanciare la run.
- **Prossimo lavoro grosso:** piano **Fase 2** (profili drone/brevetto + motore regole + pianificazione) — da scrivere con brainstorming+writing-plans.
